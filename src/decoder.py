...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        attention_out = self.self_attention(x_norm, mask)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        mask = torch.tril(torch.ones(seq_len, seq_len, device=input_ids.device), diagonal=0)
...
class TransformerForLanguageModeling(nn.Module):
    ...
    def __init__(self, vocab_size: int, max_position_embeddings: int, d_model: int,
        ...
        self.transformer_decoder = TransformerDecoder(vocab_size, max_position_embeddings, d_model,
